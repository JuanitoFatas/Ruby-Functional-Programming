...
* 平行化：如果调用函数是各自独立的，则他们可以在不同的进程甚至是机器裡执行，而不会有竞态条件的问题。“平常” 写并发程序讨厌的细节（锁、semaphore…等）在函数式编程裡面通通消失不见了。
...
让我们先釐清这一点：本文没有要提倡古怪的风格，比如仅仅为了要延续理论函数式编程的纯粹性带来的古怪风格。反之，我想说的重点是，我们应该 **当可以提昇代码的品质的时候，使用函数式编程** ，不然这个解决方法只是个烂解法而已。
